...
<template>
  <div class="new_block_content" :style="porps.fromPage == 'blockChain' ? 'margin-top:60px !important;' : ''">
    ...
    <div v-if="size === Screen.Large">
      <div>
        <el-table :data="blockDatas" table-layout="fixed" height="404px" :header-cell-style="{
          textAlign: 'center',
  ...
  // backgroundColor: '#F9F9F8',
          ...
          <el-table-column prop="height" :label="$t('home.height')">
            <template #default="scope">
              <div style="color: #0facb6; margin-bottom: 8px; font-size: 14px">{{ heights[scope.$index] }}
              </div>
              <div style="color: #6a6a69; font-size: 12px">{{ updateTimeAgo(scope.row.latest) }}
              </div>
            </template>
          </el-table-column>
          <el-table-column :label="$t('home.block_id')">
            <template #default="scope">
              <div v-for="(item, index) in scope.row.ids" :key="index">{{ item }}
              </div>
            </template>
          </el-table-column>
          <el-table-column :label="$t('home.miner')">
            <template #default="scope">
              <div v-for="(item, index) in scope.row.miners" :key="index">{{ item }}
              </div>
            </template>
          </el-table-column>
          ...
          <el-table-column :label="$t('home.message')">
            <template #default="scope">
              <div v-for="(item, index) in scope.row.msgs" :key="index">{{ item }}
              </div>
            </template>
          </el-table-column>
          <el-table-column :label="$t('home.reward')">
            <template #default="scope">
              <div v-for="(item, index) in scope.row.rewards" :key="index">{{ item }} UNC
              </div>
            </template>
          </el-table-column>
        </el-table>
    ...
    <div v-else>
      <div>
        <div v-for="(item, index) in blockDatas" :key="index">
          <div class="eco-item">
            <div class="eco-item2">
              <div class="grey-text-12-300" style="padding-right: 5px">{{ $t('home.height') }}
              </div>
              <div class="second-text-14-500">{{ heights[index] }}</div>
            </div>
            <div class="black-text-14-500">{{ updateTimeAgo(item.latest) }}
            </div>
          </div>
          <div class="eco-item">
            <div>
