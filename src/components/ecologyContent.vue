...
<template>
  <div class="ecology_content">
    <div class="ecology_header">
      ...
      <div class="ecology_time">
        <el-button size="small" :color="checkEcologyTimeType == timeItem ? '#3EDFCF' : ''"
          @click="changeTimeType(timeItem)" :style="{
    ...
    <div class="ecology_tabs">
      <div v-for="(tabsItem, tabsIndex) in [
        $t('home.defi') + '(12)',
        $t('home.community'),
        $t('home.wallet') + '(3)',
      ]" :key="tabsIndex" class="ecology_tabs_item" :class="checkEcologyTabs == tabsItem ? 'active' : ''"
    ...
    <div v-if="size !== Screen.Small">
      <div class="ecology_table">
        <el-table :data="ecoDatas" table-layout="fixed" :header-cell-style="{
          textAlign: 'center',
  ...
  // backgroundColor: '#F9F9F8',
            ...
            <template #default="scope">
              <div style="
                ...
                <div style="margin-left: 7px">
                  <div v-if="scope.row.transPer > 0" style="color: #0facb6">+{{ scope.row.transPer }}%
                  </div>
                  <div v-else-if="scope.row.transPer <= 0" style="color: palevioletred">{{ scope.row.transPer }}%
                  </div>
                </div>
              </div>
            </template>
          ...
          <el-table-column :label="$t('home.address')">
            <template #default="scope">
              <div style="
                ...
                <div style="margin-left: 7px">
                  <div v-if="scope.row.uniAddrPer > 0" style="color: #0facb6">+{{ scope.row.uniAddrPer }}%
                  </div>
                  <div v-else-if="scope.row.uniAddrPer <= 0" style="color: palevioletred">{{ scope.row.uniAddrPer }}%
                  </div>
                </div>
              </div>
            </template>
          ...
          <el-table-column :label="$t('home.lock_UNC')">
            <template #default="scope">
              <div style="
                ...
                <div style="margin-left: 7px">
                  <div v-if="scope.row.lockedPer > 0" style="color: #0facb6">+{{ scope.row.lockedPer }}%
                  </div>
                  <div v-else-if="scope.row.lockedPer <= 0" style="color: palevioletred">{{ scope.row.lockedPer }}%
                  </div>
                </div>
              </div>
            </template>
          ...
          <el-table-column label="TVL">
            <template #default="scope">
              <div style="
                ...
                <div style="margin-left: 7px">
                  <div v-if="scope.row.tvlPer > 0" style="color: #0facb6">+{{ scope.row.tvlPer }}%
                  </div>
                  <div v-else-if="scope.row.tvlPer <= 0" style="color: palevioletred">{{ scope.row.tvlPer }}%
                  </div>
                </div>
              </div>
            </template>
    ...
    <div v-else-if="size === Screen.Small">
      <div style="padding: 0 0 5px 18px" class="primary-text-14-500">{{ $t('home.rank') }}
      </div>
      <div v-for="(item, index) in ecoDatas" :key="index">
        ...
        <div class="eco-item">
          ...
          <div class="eco-merge-value">
            ...
            <div v-if="item.transPer! > 0" class="second-text-12-500">+{{ item.transPer }}%
            </div>
            <div v-else-if="item.transPer! <= 0" class="minus-text-12-500">{{ item.transPer }}%
            </div>
          </div>
        </div>
        <div class="eco-item">
          ...
          <div class="eco-merge-value">
            ...
            <div v-if="item.uniAddrPer! > 0" class="second-text-12-500">+{{ item.uniAddrPer }}%
            </div>
            <div v-else-if="item.uniAddrPer! <= 0" class="minus-text-12-500">{{ item.uniAddrPer }}%
            </div>
          </div>
        </div>
        <div class="eco-item">
          ...
          <div class="eco-merge-value">
            <div class="black-text-14-500">&nbsp;{{ item.locked }}M USD&nbsp;
            </div>
            <div v-if="item.lockedPer! > 0" class="second-text-12-500">+{{ item.lockedPer }}%
            </div>
            <div v-else-if="item.lockedPer! <= 0" class="minus-text-12-500">{{ item.lockedPer }}%
            </div>
          </div>
        </div>
        <div class="eco-item">
          ...
          <div class="eco-merge-value">
            ...
            <div v-if="item.tvlPer! > 0" class="second-text-12-500">+{{ item.tvlPer }}%
            </div>
            <div v-else-if="item.tvlPer! <= 0" class="minus-text-12-500">{{ item.tvlPer }}%
            </div>
          </div>
        </div>
        <!--        最后一条数据去除分隔线-->
