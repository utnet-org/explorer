...
const ecologyTimeTypeList = ['24h', 'week', 'month', 'year']
const checkEcologyTimeType = ref('24h')
const changeTimeType = (item: string) => {
    checkEcologyTimeType.value = item
}
//tabs相关数据及变量
const ecologyTabsList = ['DeFi(12)', 'FNS', 'Community(3)', 'DEX(3)', 'DID(3)', 'Infra(20)', 'Launchpad(1)']
        ...
        <div class="ecology_header">
            ...
            <div class="ecology_time">
                <el-button size="small" :color="checkEcologyTimeType == timeItem ? '#3EDFCF' : ''"
                    ...
                    v-for="(timeItem, timeIndex) in ecologyTimeTypeList" :key="timeIndex">{{ timeItem
                    }}</el-button>
            </div>
        </div>
