...
<template>
  <div class="computing_power_service_content">
    <div class="computing_power_service_header">
      <div class="computing_power_service_title">{{ $t('home.service_rank') }}
      </div>
    </div>
    <div class="computing_power_service_tabs">
      <div class="computing_power_service_tabs_title">{{ $t('home.effect_power') }}
      </div>
    ...
    <div v-if="size === Screen.Large">
      <div class="ecology_table">
        <el-table :data="poDatas" table-layout="fixed" :header-cell-style="{
          textAlign: 'center',
        ...
        <div v-for="(item, index) in poDatas" :key="index">
          ...
          <div class="eco-item">
            ...
            <div class="eco-merge-value">
              <div v-if="item.effPer! > 0" class="second-text-12-500">+{{ item.effPer }}%
              </div>
              <div v-else-if="item.effPer! <= 0" class="minus-text-12-500">{{ item.effPer }}%
              </div>
              <div class="black-text-14-500">&nbsp;{{ item.effPower }} TFLOPS
              </div>
            </div>
          </div>
          <div class="eco-item">
